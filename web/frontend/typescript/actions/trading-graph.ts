...
export function tradingGraph() {
    ...
    for (let i = 0; i < tradingAssets.length; i++) {
        ...
        tradingAssets[i].addEventListener('click', (e) => {
            ...
            currentAsset = asset;
            wsInstance.emit('get-history', asset);
            ...
            // wsInstance.removeAllListeners('asset-updated');
            clearGraph();
        });
    }
    ...
    wsInstance.on('history-updated', (historyData) => {
        data = data.splice(0, data.length);
        historyData = historyData
            .map(item => {
                item = JSON.parse(item);
                const date = new Date(item.timestamp);
                return {date: date, price: item.value};
            })
            .reverse()
    ...
    wsInstance.on('asset-updated', (asset) => {
        ...
        if (data.length > 0 && emptyLabel !== null) {
            emptyLabel.text('');
        }
...
function drawGraph() {
    ...
    y.domain([0, d3.min(data, function (d) {
        return d.price;
    })]);
    x2.domain(x.domain());
    y2.domain(y.domain());
